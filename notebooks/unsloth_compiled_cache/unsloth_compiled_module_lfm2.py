...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.lfm2.modeling_lfm2 import (__name__, F, Callable, Optional, Union, torch, nn, Cache, GenerationMixin, use_kernel_forward_from_hub, BaseModelOutputWithPast, CausalLMOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, TransformersKwargs, can_return_tuple, deprecate_kwarg, Lfm2Config, causal_conv1d_fn, causal_conv1d_update, Lfm2HybridConvCache, is_fast_path_available, Lfm2PreTrainedModel, Lfm2Model, Lfm2ForCausalLM)

@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def Lfm2RMSNorm_forward(self, hidden_states):
...
class Lfm2RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        return Lfm2RMSNorm_forward(self, hidden_states)
...
class Lfm2RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return Lfm2RotaryEmbedding_forward(self, x, position_ids)
...
class Lfm2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return Lfm2Attention_forward(self, hidden_states, position_embeddings, attention_mask, past_key_values, cache_position, **kwargs)
...
class Lfm2ShortConv(nn.Module):
    ...
    def forward(
        ...
        return Lfm2ShortConv_forward(self, hidden_states, past_key_values, cache_position, attention_mask)


@torch.compiler.disable(recursive = False)
@can_return_tuple
def Lfm2ForCausalLM_forward(
    self,
...
class Lfm2ForCausalLM(Lfm2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Lfm2ForCausalLM_forward(self, input_ids, attention_mask, position_ids, past_key_values, inputs_embeds, labels, use_cache, cache_position, logits_to_keep, **kwargs)
